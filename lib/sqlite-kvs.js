...
class KVS {
  ...
  async open(dbpath) {
    ...
    return db.serialize(async () => {
      ...
      this.stmt_find.arun = util.promisify(this.stmt_find.all);
      ...
      this.stmt_purge = db.prepare(
        'DELETE FROM items WHERE key LIKE ?'
      );
      this.stmt_purge.arun = util.promisify(this.stmt_purge.run);
    });
  };
  ...
  async delete(key) {
    await this.stmt_delete.arun([key]);
    return this;
  };
  
  async purge(prefix) {
    await this.stmt_purge.arun([prefix+'%']);
    return this; 
  };
